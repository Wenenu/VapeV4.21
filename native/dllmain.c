...
static int install_modular_payload_loader(
        ...
        jclass url_class, jobject url, const wchar_t *jar_path) {
    ...
    payload_path = new_wide_string(env, jar_path);
    payload_loader = payload_loader_init == NULL || payload_path == NULL ? NULL
            : (*env)->NewObject(env, payload_loader_class,
                    payload_loader_init, payload_path, *loader);
...
static int add_jar_to_loader(
    ...
    modular_result = install_modular_payload_loader(
            env, loader, url_loader_class, url_class, url, jar_path);
    if (modular_result != 0) {
